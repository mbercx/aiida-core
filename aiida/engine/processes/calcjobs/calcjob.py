...
def validate_stash_options(stash_options: Any, _: Any) -> Optional[str]:
    ...
    # Normally we would check if `stash_options` is not `None, but due to a bug in `plumpy`, even if not specified, the
    # validator will get called and `stash_options` will be an empty tuple
    if not stash_options:
        return
